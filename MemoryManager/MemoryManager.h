...
#	define MM_MALLOC_SIMD(size, category) ::MM::AlignedAllocationPolicy<category>::AllocateAlignedBytes(size, typeid(category).name(), __FILE__, __LINE__, __FUNCTION__)
/// Allocate a block of raw memory aligned to user defined boundaries, and indicate the category of usage
#	define MM_MALLOC_ALIGN(size, category, align) ::MM::AlignedAllocationPolicy<category, align>::AllocateAlignedBytes(size, typeid(category).name(), __FILE__, __LINE__, __FUNCTION__)
/// Allocate a block of memory for a primitive type aligned to SIMD boundaries, and indicate the category of usage
#	define MM_ALLOC_T_SIMD(T, count, category) static_cast<T*>(::MM::AlignedAllocationPolicy<category>::AllocateAlignedBytes(sizeof(T)*(count), typeid(category).name(), __FILE__, __LINE__, __FUNCTION__))
/// Allocate a block of memory for a primitive type aligned to user defined boundaries, and indicate the category of usage
#	define MM_ALLOC_T_ALIGN(T, count, category, align) static_cast<T*>(::MM::AlignedAllocationPolicy<category, align>::AllocateAlignedBytes(sizeof(T)*(count), typeid(category).name(), __FILE__, __LINE__, __FUNCTION__))
...
#	define MM_FREE_SIMD(p, category) ::MM::AlignedAllocationPolicy<category>::DeallocateAlignedBytes(p)
...
#	define MM_FREE_ALIGN(p, category, align) ::MM::AlignedAllocationPolicy<category, align>::DeallocateAlignedBytes(p)

/// Allocate space for one primitive type, external type or non-virtual type aligned to SIMD boundaries
#	define MM_NEW_T_SIMD(T, category) new (::MM::AlignedAllocationPolicy<category>::AllocateAlignedBytes(sizeof(T), typeid(category).name(), __FILE__, __LINE__, __FUNCTION__)) T
...
#	define MM_NEW_ARRAY_T_SIMD(T, count, category) ::MM::ConstructArray(static_cast<T*>(::MM::AlignedAllocationPolicy<category>::AllocateAlignedBytes(sizeof(T)*(count), typeid(category).name(), __FILE__, __LINE__, __FUNCTION__)), count) 
/// Free the memory allocated with MM_NEW_T_SIMD. Category is required to be restated to ensure the matching policy is used
#	define MM_DELETE_T_SIMD(p, T, category) if (p) { (p)->~T(); ::MM::AlignedAllocationPolicy<category>::DeallocateAlignedBytes(p); }
...
#	define MM_DELETE_ARRAY_T_SIMD(p, T, count, category) if (p) { for (size_t i = 0; i < count; ++i) { (p)[i].~T(); } ::MM::AlignedAllocationPolicy<category>::DeallocateAlignedBytes(p); }
/// Allocate space for one primitive type, external type or non-virtual type aligned to user defined boundaries
#	define MM_NEW_T_ALIGN(T, category, align) new (::MM::AlignedAllocationPolicy<category, align>::AllocateAlignedBytes(sizeof(T), typeid(category).name(), __FILE__, __LINE__, __FUNCTION__)) T
...
#	define MM_NEW_ARRAY_T_ALIGN(T, count, category, align) ::MM::ConstructArray(static_cast<T*>(::MM::AlignedAllocationPolicy<category, align>::AllocateAlignedBytes(sizeof(T)*(count), typeid(category).name(), __FILE__, __LINE__, __FUNCTION__)), count) 
/// Free the memory allocated with MM_NEW_T_ALIGN. Category is required to be restated to ensure the matching policy is used
#	define MM_DELETE_T_ALIGN(p, T, category, align) if (p) { (p)->~T(); ::MM::AlignedAllocationPolicy<category, align>::DeallocateAlignedBytes(p); }
...
#	define MM_DELETE_ARRAY_T_ALIGN(p, T, count, category, align) if (p) { for (size_t i = 0; i < count; ++i) { (p)[i].~T(); } ::MM::AlignedAllocationPolicy<category, align>::DeallocateAlignedBytes(p); }
...
// Aligned allocation
/// Allocate a block of raw memory aligned to SIMD boundaries, and indicate the category of usage
#	define MM_MALLOC_SIMD(size, category) ::MM::AlignedAllocationPolicy<category>::AllocateAlignedBytes(size)
/// Allocate a block of raw memory aligned to user defined boundaries, and indicate the category of usage
#	define MM_MALLOC_ALIGN(size, category, align) ::MM::AlignedAllocationPolicy<category, align>::AllocateAlignedBytes(size)
/// Allocate a block of memory for a primitive type aligned to SIMD boundaries, and indicate the category of usage
#	define MM_ALLOC_T_SIMD(T, count, category) static_cast<T*>(::MM::AlignedAllocationPolicy<category>::AllocateAlignedBytes(sizeof(T)*(count)))
/// Allocate a block of memory for a primitive type aligned to user defined boundaries, and indicate the category of usage
#	define MM_ALLOC_T_ALIGN(T, count, category, align) static_cast<T*>(::MM::AlignedAllocationPolicy<category, align>::AllocateAlignedBytes(sizeof(T)*(count)))
/// Free the memory allocated with either MM_MALLOC_SIMD or MM_ALLOC_T_SIMD. Category is required to be restated to ensure the matching policy is used
#	define MM_FREE_SIMD(p, category) ::MM::AlignedAllocationPolicy<category>::DeallocateAlignedBytes(p)
/// Free the memory allocated with either MM_MALLOC_ALIGN or OGRE_MM_T_ALIGN. Category is required to be restated to ensure the matching policy is used
#	define MM_FREE_ALIGN(p, category, align) ::MM::AlignedAllocationPolicy<category, align>::DeallocateAlignedBytes(p)
...
/// Allocate space for one primitive type, external type or non-virtual type aligned to SIMD boundaries
#	define MM_NEW_T_SIMD(T, category) new (::MM::AlignedAllocationPolicy<category>::AllocateAlignedBytes(sizeof(T))) T
/// Allocate a block of memory for 'count' primitive types aligned to SIMD boundaries - do not use for classes that inherit from AllocatedObject
#	define MM_NEW_ARRAY_T_SIMD(T, count, category) ::MM::ConstructArray(static_cast<T*>(::MM::AlignedAllocationPolicy<category>::AllocateAlignedBytes(sizeof(T)*(count))), count) 
/// Free the memory allocated with MM_NEW_T_SIMD. Category is required to be restated to ensure the matching policy is used
#	define MM_DELETE_T_SIMD(p, T, category) if (p) { (p)->~T(); ::MM::AlignedAllocationPolicy<category>::DeallocateAlignedBytes(p); }
/// Free the memory allocated with MM_NEW_ARRAY_T_SIMD. Category is required to be restated to ensure the matching policy is used, count and type to call destructor
#	define MM_DELETE_ARRAY_T_SIMD(p, T, count, category) if (p) { for (size_t i = 0; i < count; ++i) { (p)[i].~T(); } ::MM::AlignedAllocationPolicy<category>::DeallocateAlignedBytes(p); }
/// Allocate space for one primitive type, external type or non-virtual type aligned to user defined boundaries
#	define MM_NEW_T_ALIGN(T, category, align) new (::MM::AlignedAllocationPolicy<category, align>::AllocateAlignedBytes(sizeof(T))) T
/// Allocate a block of memory for 'count' primitive types aligned to user defined boundaries - do not use for classes that inherit from AllocatedObject
#	define MM_NEW_ARRAY_T_ALIGN(T, count, category, align) ::MM::ConstructArray(static_cast<T*>(::MM::AlignedAllocationPolicy<category, align>::AllocateAlignedBytes(sizeof(T)*(count))), count) 
/// Free the memory allocated with MM_NEW_T_ALIGN. Category is required to be restated to ensure the matching policy is used
#	define MM_DELETE_T_ALIGN(p, T, category, align) if (p) { (p)->~T(); ::MM::AlignedAllocationPolicy<category, align>::DeallocateAlignedBytes(p); }
/// Free the memory allocated with MM_NEW_ARRAY_T_ALIGN. Category is required to be restated to ensure the matching policy is used, count and type to call destructor
#	define MM_DELETE_ARRAY_T_ALIGN(p, T, count, category, align) if (p) { for (size_t i = 0; i < count; ++i) { (p)[i].~T(); } ::MM::AlignedAllocationPolicy<category, align>::DeallocateAlignedBytes(p); }
